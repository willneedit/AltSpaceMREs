...
export class SGDBpg implements SGDBLike {
    ...
    public async init() {
        ...
        try {
            ...
            this.db = null;
            return Promise.reject("Postgres database down or unconfigured (1).");
        }
        ...
        await this.db.query('CREATE TABLE IF NOT EXISTS known_locations (' +
            ...
            'PRIMARY KEY (lid,gid) )').then((res: QueryResult) => {
                ...
                console.debug('Database unresponsive.');
                return Promise.reject("Postgres database down or unconfigured (2).");
            });
